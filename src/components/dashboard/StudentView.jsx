...
const StudentView = ({ user }) => {
  ...
  // Load library messages for notifications
  const studentLibMessages = loadData(`studentLibMessages_${user.name}`, []);

  const now = new Date();
  const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
  const hasLibraryNotifications = studentLibMessages.some(msg =>
    new Date(msg.date) > sevenDaysAgo
  );

  const hasNewLibraryMsgs = hasLibraryNotifications && studentLibMessages.some(m => !m.type || m.type === 'library');
  const hasNewKuthbkhanaMsgs = hasLibraryNotifications && studentLibMessages.some(m => m.type === 'kuthbkhana');

  const NavItem = ({ value, label, icon: Icon, hasNotification }) => (
    <Button
      variant={activeTab === value ? "secondary" : "ghost"}
      className={`w-full justify-start text-sm h-11 relative ${
        activeTab === value
      ...
      case 'library':
        return (
          <StudentLibraryTab
            user={user}
            libraryType="library"
            initialBooks={initialBookData}
            messages={studentLibMessages}
          />
        );
      case 'kuthbkhana':
        return (
          <StudentLibraryTab
            ...
            initialBooks={initialBookData.filter(b => b.isArabic)}
            messages={studentLibMessages}
          />
        );
      default:
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <nav className="w-52 md:w-56 border-r border-border/50 p-3 space-y-1.5 bg-background/30 dark:bg-background/20 hidden sm:block">
        ...
        <NavItem
          ...
          label="Library"
          icon={LibraryIcon}
          hasNotification={hasNewLibraryMsgs}
        />
        <NavItem
          value="kuthbkhana"
          label="Kuthbkhana"
          icon={BookOpen}
          hasNotification={hasNewKuthbkhanaMsgs}
        />
      </nav>
        ...
        <Select value={activeTab} onValueChange={setActiveTab}>
          ...
          <SelectContent>
            ...
            <SelectItem value="library">
              Library {hasNewLibraryMsgs && "(New)"}
            </SelectItem>
            <SelectItem value="kuthbkhana">
              Kuthbkhana {hasNewKuthbkhanaMsgs && "(New)"}
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
